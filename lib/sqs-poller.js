  ...
  return Promise.map(chunked, (entryChunk) => new Promise((ok, err) =>
    ...
    .tap(res => {
      if (res.data.Failed && res.data.Failed.length) {
        var fails = _.map(res.data.Failed, it => it.Id = it.Id.substring(it.Id.length - 10));
        error(`VisTimeoutUpdater ${this._id} failed to update visibility timeout for ${res.data.Failed.length} messages: ${JSON.stringify(fails)}`);
      }
    }))
