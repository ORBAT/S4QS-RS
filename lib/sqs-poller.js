...
Poller.prototype._rcv = function () {
  return new Promise((resolve, reject) => {
    ...
    this._logger.trace("Polling for new messages");
    req.on('success', (resp) => {
      if (resp && resp.data) {
        const msgs = resp.data.Messages || [];
        this._logger.trace({nMsgs: msgs.length}, "Got new messages");
...
VisTimeoutUpdater.prototype._removeFailed = function (maxFails) {
  const failed = _.filter(_.values(this._msgMap), it => it._nTimesFailed > maxFails)
    , failedUris = _.flatten(failed.map(it => {
      this.removeMsg(it);
      return it.toURIs();
    }))
    ;
  ...
  if (failedUris.length) {
    this._logger.warn({uris: failedUris}, "Updating visibility timeout for these messages has failed too many times; stopping it");
  }
};
...
VisTimeoutUpdater.prototype._updateLoop = function() {
  ...
  this._logger.trace({nMsgs: entries.length, VisibilityTimeout: this._visibilityTimeout}, "Updating visibility timeouts");
  ...
  return Promise.map(chunked, (entryChunk) => new Promise((ok, err) =>
    ...
    .then(res => _.get(res, "data.Failed") || [])
  )
    .then(_.flatten)
    .tap(fails => {
      if (fails && fails.length) {
        ...
        this._logger.warn({nOk:entries.length - fails.length, nErrors: fails.length, uris: ut.messagesToURIs(badMsgs), errorCodes: _.uniq(_.pluck(fails, "Code"))},
          "Error updating visibility timeouts");
      }
    })
    .tap(() => this._logger.trace("Visibility timeouts updated"))
    .delay(this._interval)
    .tap(() => setImmediate(this._updateLoop.bind(this)));
};
