  ...
  return Promise.map(chunked, (entryChunk) => new Promise((ok, err) =>
    ...
    .tap(fails => {
      if (fails && fails.length) {
        ...
        this._logger.warn({nOk:entries.length - fails.length, nErrors: fails.length, uris: ut.messagesToURIs(badMsgs),messages: _.uniq(_.pluck(fails, "Message")), errorCodes: _.uniq(_.pluck(fails, "Code"))},
