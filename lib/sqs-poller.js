...
var Poller = module.exports.Poller = function Poller(sqs, options) {
  ...
  var incr = rcvInstr.instrumenterFn("increment", "msgs.count");

  this._inflightMsgCount = this._instrumenter.instrumenterFn("gaugeDelta", "msgs.inflight");

  var rcv = rcvInstr.instrumentCalls("rcv", this._rcv.bind(this), {count: true});

  this._msgSource = $(function (push, next) {
    try {
      push(null, $(rcv().delay(this._pollIntervalS * 1000)));
      next();
    } catch (e) {
...
Poller.prototype.deleteMsgs = function(messages) {
  ...
  return Promise.map(chunked, function (entryChunk) {
    ...
    .tap(function() {
      this._inflightMsgCount(-1 * messages.length);
    }.bind(this))
