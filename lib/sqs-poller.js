...
var Poller = module.exports.Poller = function Poller(sqs, options) {
  ...
  this._instrumenter = new instr.Instrumenter(topStatsd, "sqs-poller");
  ...
  this._unknownMsgs = this._msgSource.observe()
    ...
    .doto((msgs) => {
      debug(`${this._id} ignoring ${msgs.length} messages I don't know how to handle: URIs were ${_.map(msgs, ut.send("toURIs")).join(",")}`);
    })
  ...
  this._deletionStream
    ...
    .errors(err => error(`${this._id} had an error deleting messages: ${err}`))
    .each(() => {}); // make sure the stream gets thunked
  ...
  return new Promise((resolve, reject) => {
    ...
    req.on('success', (resp) => {
      if (resp && resp.data) {
        ...
        var enriched = _.map(msgs, msg => new EnrichedMessage(msg, this));
...
VisTimeoutUpdater.prototype._markFailed = function(msgs) {
  _.each(msgs, msg => this._msgMap[msg.ReceiptHandle]._nTimesFailed++)
};

VisTimeoutUpdater.prototype._removeFailed = function (maxFails) {
  var failed = _.filter(_.values(this._msgMap), it => it._nTimesFailed > maxFails);

  _.each(failed, it => {
    error(
      `VisTimeoutUpdater tried to unsuccessfully update message with URI ${it.toURIs()} ${it._nTimesFailed} times. Updates will be stopped`);
    this.removeMsg(it);
  });
};
...
VisTimeoutUpdater.prototype._updateLoop = function() {
  ...
  return Promise.map(chunked, (entryChunk) => new Promise((ok, err) =>
    ...
    .catch(e => {
      error(`VisTimeoutUpdater ${this._id} error updating visibility timeouts for ${entryChunk.length} messages: ${e}`);
      return {data: {}}
    })
    .tap(res => {
      var fails = res.data.Failed;
      if (fails && fails.length) {
        var badMsgs = _.reduce(fails, (acc, it) => {
          var idx = Number.parseInt(it.Id);
          if (!_.isNaN(idx)) {
             acc.push(msgs[idx])
          }
          return acc;
        }, []);
        error(`VisTimeoutUpdater ${this._id} failed to update visibility timeout for ${fails.length} messages: ${JSON.stringify(fails)}. URIs were ${ut.messagesToURIs(badMsgs)}`);
