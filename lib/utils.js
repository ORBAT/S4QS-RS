...
var PgConn = require('pg/lib/connection');
 ...
 */
exports.getPgClient = function getPgClient(pg, connString, logger) {
  var _done;
  if(!logger) {
    return Promise.reject(new Error("no logger provided to getPgClient"));
  }
  return pg.connectAsync(connString)
    .spread((client, done) => {
      _done = done;
      client.logger = logger.child({pgProcessId: client.processID});
      return client;
    })
...
function inTransaction(client, ctx, fn) {
  ...
  client.txLogger = client.logger.child({txId: txId, txCtx: ctx});
  client.txLogger.trace("Opening transaction");


  return client.queryAsync("begin").return(client)
    .tap(() => client.txLogger.debug("Transaction open"))
    .then(fn)
    .then((res) => {
        client.txLogger.trace("Committing transaction");
        return client.queryAsync("commit")
          .tap(() => client.txLogger.debug("Committed transaction"))
          .return(res);
      }
      , (err) => {
        if (err instanceof Promise.TimeoutError) {
          client.txLogger.error("Timeout inside transaction");
          throw err;
        }

        if (/connection terminated/i.test(err.message)) {
          client.txLogger.error("Connection terminated, not doing ROLLBACK");
          throw err;
        }


        client.txLogger.error({err: err}, "ROLLBACK due to error");
        return client.queryAsync("rollback")
          .tap(() => client.txLogger.info("ROLLBACK done"))
          .throw(err);
      });
}
...
function cancelClient(client) {
  client.logger.warn("Cancel requested for DB client");
  return new Promise((resolve, reject) => {
    var con = new PgConn();

    con.once("connect", () => {
      con.once("end", () => {client.logger.info("Cancel done"); resolve()});
      client.logger.info("Sending cancel");
...
/**
 * takes a function fn, and returns a function that takes an array and maps it using fn. Basically a partial right
 * application of _.map
 * @example
 * splat(n => n+1)([1,2,3,4]) // returns [2,3,4,5]
 * @param fn
 * @returns {Function}
 */
function splat(fn) {
 ...
 * elements, i.e. _.map(array, it => it.toString());
 * @returns {Function}
 */
function send () {
...
var eventToS3URIs = exports.eventToS3URIs = function eventToS3URIs(evt) {
  ...
  if (evt.Type === "Notification") {
    throw new Error("Suspicious SQS message. Are you sure the SNS message delivery is set to raw?");
  }
  ...
  if (_.isArray(evt.Records) && evt.Records.length > 0) {
    return _.reduce(evt.Records, (acc, record) => {
      ...
      if (record.eventVersion == "2.0" && s3 && s3.s3SchemaVersion == "1.0") {
        ...
        throw new Error("Unknown event (" + record.eventVersion + ") or S3 event version (" +
              ((s3 && s3.s3SchemaVersion) || "N/A") + ")");
      }
      return acc;
    }, []);
  } else {
    throw new Error(`Event had no Records? ${JSON.stringify(evt)}`);
  }
};
...
/**
 * Runs an array of functions returning promises in sequence, so that each function call waits for the previous one to
 * finish. A Promise of an array of the return values of the functions is returned
 * @param {Array} fns array of functions that'll return a Promise when called
 * @returns {Promise} a promise that'll be fulfilled with the return values of fns
 */
