...
function withTransaction(client, trigger) {
  if(!trigger) {
    trigger = {shouldCommit: true};
  }
  var txId = ((Date.now() / 1000).toFixed(0)) + exports.randomString(3);

  debug("Beginning transaction " + txId);
    ...
    .disposer(function(client, promise) {
      if(promise.isFulfilled() && trigger.shouldCommit) {
        return client.queryAsync("commit").tap(function() {
          debug("Committed transaction " + txId);
        });
      } else {
        error("ROLLBACK. Not committing transaction " + txId + ": " + trigger.reason);
        return client.queryAsync("rollback").tap(function() {
          debug("Rolled back transaction " + txId);
        });
      }
    });
}
