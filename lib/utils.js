...
function withTransaction(client, trigger) {
  if(!trigger) {
    trigger = {shouldCommit: true, context: "no context", reason: "[N/A]" +
                                                                  ""};
  }
  var txId = ((Date.now() / 1000).toFixed(0)) + exports.randomString(3);

  debug("Beginning transaction " + txId + " (" + trigger.context + ") " + client.processID);
  return client.queryAsync("begin").return(client)
    .disposer(function(client, promise) {
      if(promise.isFulfilled() && trigger.shouldCommit) {
        return client.queryAsync("commit").tap(function() {
          debug("Committed transaction " + txId + " (" + trigger.context + ") " + client.processID);
        });
      } else {
        error("ROLLBACK. Not committing transaction " + txId + ": " + trigger.reason + "(" + trigger.context + ") "
              + client.processID);
        return client.queryAsync("rollback").tap(function() {
          debug("Rolled back transaction " + txId + "(" + trigger.context + ") " + client.processID);
        });
      }
    });
}
