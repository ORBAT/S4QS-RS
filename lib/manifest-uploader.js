...
var Uploader = exports.Uploader = function Uploader(s3, options) {
  ...
  this._instrumenter = new instr.Instrumenter(topStatsd, "manif-uploader");
  ...
  this.outputStreams = _.reduce(this.inputStreamMap, (acc, stream, baseName) => {
    var str = stream.fork().filter((msg) => {
        ...
        if (!ours) {
          var errMsg = `Stream for ${baseName} got a message for ${groups[0]} (# groups for message was ${groups.length}): this shouldn't happen`;
          error(errMsg);
          throw new Error(errMsg);
        }
        return ours;
      })
      .through(_.partial(this.msgsToManifests.bind(this), baseName));
    acc[baseName] = str;
    return acc;
  }, {})
...
Uploader.prototype.msgsToManifests = function(baseName, msgStream) {
  ...
  debug(`started msgsToManifests for base name ${baseName}`);
  return msgStream
    .batchWithTimeOrCount(this._maxWaitSeconds * 1000, this._maxToUpload)
    .map(gauger)
    .map((msgs) => {
      debug(`msgsToManifests got ${msgs.length} messages for base name ${baseName}. URIs are ${ut.messagesToURIs(msgs).join(",")}`);
      perGroupIncr(baseName, msgs.length);
      var mf = this._newManifest(baseName);
      mf._addAll(msgs);
