...
var Uploader = exports.Uploader = function Uploader(s3, options) {
  ...
  this.outputStreams = _.reduce(this._baseNames, (acc, base) => {
    var str = this.inputStream.fork().filter((msg) => {
        var groups = _.map(_messageToURIs(msg), this._grouper);
        var ours = groups.length == 1 && groups[0] == base;
        //debug(`inputstream filter for base name ${base}, uris ${groups.join(",")}, ours ${ours}`);
        return ours;
      })
...
/*    .doto((msg) => {
      ...
      var group= _.map(uris, this._grouper)[0];
      if(_.isUndefined(grpCounts[group])) {
        grpCounts[group] = 0;
      }
      grpCounts[group]++;
      debug(`msgsToManifests.msgStream grp ${group}, grp count ${grpCounts[group]}`)}
    )*/
    .batchWithTimeOrCount(this._maxWaitSeconds * 1000, this._maxToUpload)
    .doto((msgs) => debug(`batched msgs ${msgs.length}`))
    .map(gauger)
    .map((msgs) => {
      debug(`msgsToManifests call`);
