 ...
 * @param {Function} options.grouper Function that takes an SQS message and turns it into a table name. Used for
 * grouping SQS messages into the correct manifest.
 * @constructor
 * @type {Uploader}
 */
...
Uploader.prototype._newManifest = function _newManifest() {
  var manifest = new Manifest(this._mandatory, this._bucket, this._prefix, this._s3);
  debug("Created new manifest with URI " + manifest.manifestURI);
  return  manifest;
};

Uploader.prototype.addMessages = function addMessage(msgs) {
  this._currentManifest._addAll(msgs);
  if(this._currentManifest.length >= this._minToUpload) {
    this._uploadCurrent();
  }
};
 ...
 * @type {Manifest}
 */
var Manifest = exports.Manifest = function Manifest(mandatory, bucket, prefix, s3, table) {
...
Manifest.prototype._push = function _push(msg) {
  this._msgs.push(msg);
  this._uris = this._uris.concat(_toUris([msg]));
  return this._uris.length;
};
