...
Manifest.prototype.done = function done(successful) {
  return new Promise((resolve, reject) => {
    if(this._alreadyMoved) {
      this._logger.info("Manifest has already been moved");
    ...
    req.on("success", () => {
      this._logger.trace({destPath: destPath}, "Move done");
      this._alreadyMoved = true;
      resolve(this.delete());
    });

    req.on("error", (err) => {
      this._logger.warn({destPath: destPath, err: err}, "Moving failed");
      reject(err);
    });
