...
HealthChecker.prototype.start = co.wrap(function* start() {
  /*
      BUG(ORBAT): if the initial check due to an exception in _checkStatus, s4qs will keep the health bad until the watcher
      in _startWatcher is triggered.

      TODO(ORBAT): if _checkStatus() fails due to a server error, try again periodically
   */
