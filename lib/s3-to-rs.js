...
S3Copier.prototype._connAndCopy = function _connAndCopy(s3URI, base, table) {
  ...
  function _dedupCopy(client) {
    ...
    // get the latest finalized time, or barring that
    var latestFinalizedP = client.queryAsync(util.format("SELECT COALESCE(MAX(finalized_time), " +
                                                         "sysdate - interval '%s') FROM %s_tracking",
      dedupConf.dropOlderThan || '1 hour', table));

    return latestFinalizedP.then(ut.redshiftDateFix).then(function (latestFinalized) {
      debug("Latest finalization time was " + latestFinalized.toUTCString());
      return client.queryAsync(stagingDdl)
