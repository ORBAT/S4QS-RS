...
var S3Copier = exports.S3Copier = function S3Copier(poller, pg, s3, rs, copyParams, options) {
  ...
  if(!options.timeSeries && !options.tableConfig) {
  ...
  this._tblMgr = new TableManager(this._usingPg, copyParams.schema, this._tablePostfix, options.timeSeries
                                                                                        || options.tableConfig);
...
S3Copier.prototype._connAndCopy = function _connAndCopy(s3URI,table) {
  ...
  return this._usingPg(function (client) {
    return ut.inTransaction(client, txCtx, function (client) {
      ...
      var query = util.format(this._copyTemplate, table, s3URI);
      debug("Starting COPY query for " + s3URI + " to " + table);
...
S3Copier.prototype._onManifest = function _onManifest(manif) {
  ...
  var handle = function _handle() {
    ...
    var doCopy = _.partial(this._connAndCopy.bind(this), manif.manifestURI);
...
var _dedupDdl = "CREATE TABLE IF NOT EXISTS %s_dedup (%s)"; // 1: base+postfix, 2: id col name + type, 3: ts column name

var _isPkRe = /primary key/i;
 ...
 * @private
 */
TableManager.prototype._getPkFor = function(base) {
  var cols = this._options[base].columns;
  for(var i = 0; i < cols.length; ++i) {
    if(_isPkRe.test(cols[i])) {
...
TableManager.prototype._createMetadataTbls = Promise.method(function() {
  ...
  /*
   CREATE TABLE IF NOT EXISTS [table]_dedup (
   event_id VARCHAR(24) DISTKEY,
   ts TIMESTAMP SORTKEY
   );

   CREATE TABLE IF NOT EXISTS [table]_tracking (
   processing_time TIMESTAMP,
   finalized_time TIMESTAMP SORTKEY
   );


   CREATE TABLE IF NOT EXISTS [table]_errors (
   dedup_event_id VARCHAR(24),
   processing_time TIMESTAMP SORTKEY
   -- , [columns from column config...]
   );
   */
