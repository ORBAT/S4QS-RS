...
S3Copier.prototype._setupPollers = function _setupPollers(options) {
  ...
  const seen = msg => _.any(msg.toURIs(), this._URISeenCache.get.bind(this._URISeenCache));
...
S3Copier.prototype._stagingToActual = function _stagingToActual(client, table, base, stagingTbl, latestFinalized) {
  ...
  let query = `INSERT INTO ${table} SELECT ${columns.join(", ")} FROM `
              +
  ...
  const sortKey = this._tblMgr.sortKeyFor(base);
  ...
  return client.queryAsync(query).tap((res) => {
    const logObj = {baseName: base, rowCount: res.rowCount};
    if (sortKey) logObj.sortKey = sortKey;
    client.txLogger.debug(logObj, "Inserted from staging to actual table");
  })
    .get("rowCount")
...
const _copyParamsTempl = exports._copyParamsTempl = function _copyParamsTempl(copyParams) {
  ...
  const withp = _.pairs(copyParams.withParams).reduce((acc, pair) => {
    const key = pair[0];
    let value = pair[1];
    if (!(_.isNumber(value) || _boolish.test(value.toString()) || _enc.test(key))) {
...
const _pkRe = /\bprimary key\b/i
  , _sortKeyRe = /sortkey\((.*?)\)/i
  ;

TableManager.prototype.sortKeyFor = function(base) {
  const attrs = this._options[base].tableAttrs;
  const maybeSortkey = _.get(attrs.filter(attr => _sortKeyRe.test(attr)), "0");
  if(maybeSortkey) {
    return _sortKeyRe.exec(maybeSortkey)[1];
  }
  return null;
};
...
TableManager.prototype._columnDefinition = function(baseName, colName) {
...
TableManager.prototype._genAddColumns = function(baseName, toAdd) {
  ...
  return dropSetup.map(([tbl, col]) => `ALTER TABLE ${tbl} ADD COLUMN ${this._columnDefinition(baseName, col)}`)
};
...
TableManager.prototype.updateTableSchemas = co.wrap(function* () {
  ...
  if (toBeAltered.length) {
    ...
    if (!this._enableSchemaEvolution) {
      this._logger.warn({statements: _.values(alters)},
        "Schema evolution is not enabled, but these are the statements that would be run");
      return toBeAltered;
    }
