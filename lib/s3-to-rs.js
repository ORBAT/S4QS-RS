...
var S3Copier = exports.S3Copier = function S3Copier(poller, pg, s3, rs, copyParams, options) {
  ...
  this._timeSeriesMgr = new TimeSeriesManager(this._usingPg, copyParams.schema, options.timeSeries);
...
S3Copier.prototype._onManifest = function _onManifest(manif) {
  var pfx = this._tablePostfix ? this._tablePostfix : "";
  debug("Handling manifest " + manif.manifestURI + " for table " + manif.table + pfx);

  var table = manif.table;

  if(this._tablePostfix) {
    table += this._tablePostfix;
  }

  // TODO: add time series postfix

  var promise = this._connAndCopy(manif.manifestURI, table).bind(this)
      .then(function (uri) {
  ...
  if(this._manifestsPending[manif.table] && this._manifestsPending[manif.table].isPending()) {
    error("Already have a pending manifest upload for table " + manif.table + pfx + "? Joining it anyhow");
 ...
 * @param usingPgFn A function that takes another function which will be called with a promisified Postgres client
 * and returns a promise of a result
 * @constructor
 */
var TimeSeriesManager = exports.TimeSeriesManager = function TimeSeriesManager(usingPgFn, schema, options) {
...
TimeSeriesManager.prototype._listTsTables = function(name) {
  ...
  return this._usingPg(function (client) {
    return client.queryAsync("select tablename from pg_tables where schemaname = '$1'" +
                             " and tablename like '$2' || '_%' order by tablename asc",
        [this.schema, name])
      .then(function (res) {
...
/**
 * Checks the amount of current time series tables for a base name, prunes them if necessary.
 * @return {Array} Array of deleted time series table names, if any.
 * @private
 */
  ...
  return this._listTsTables(name).bind(this)
    .then(function(tables) {
      ...
      if(tables.length > tsOpts.maxTables) {
        ...
        var toDelete = tables.slice(0, diff);
        return Promise.map(toDelete, this._dropTable.bind(this));
      }
      return [];
    })
    ;
...
TimeSeriesManager.prototype.tsTableFor = Promise.method(function (name) {
  ...
  var now = Date.now()
    , postf = "_" + ((now - now % (tsOpts.period * 1000)) / 1000).toFixed(0)
    , tsTableName = this.schema + "." + name + postf
  ...
  return this._usingPg(function (client) {
    ...
    .then(function () {
      ...
      // TODO: check total amount of ts tables, delete old ones if necessary
      // TODO: update view
      return this._pruneTsTables(name)
        .tap(function(dels) {
          if(dels.length > 0) {
