...
S3Copier.prototype._updateDedupTbl = function _updateDedupTbl(client, table, base, stagingTbl, latestFinalized) {
  ...
  var dedupConf = this._dedupConfsFor(base)
    ...
    , dropOlder = util.format("OR %s.%s < ('%s'::timestamp)", stagingTbl, tsc, latestFinalized)
...
S3Copier.prototype._stagingToActual = function _stagingToActual(client, table, base, stagingTbl, latestFinalized) {
  ...
  var dedupConf = this._dedupConfsFor(base)
    ...
    , pk = this._tblMgr.pkFor(base).name
    , dropOlder = util.format("AND staging.%s >= ('%s'::timestamp)", dedupConf.timestampCol, latestFinalized)
    , query = util.format(
      "INSERT INTO %s SELECT %s FROM "
      + "(SELECT %s.*, ROW_NUMBER() OVER (PARTITION BY %s.%s) AS id_rank, dedup.%s AS dedup_id "
      + "FROM %s "
...
S3Copier.prototype._connAndCopy = function _connAndCopy(s3URI, base, table) {
  ...
  function _lockTables(client) {
    var q = util.format("LOCK %s_tracking, %s_dedup, %s_errors", table, table, table);
    debug(q);
  ...
  function _dedupCopy(client) {
    ...
    var latestFinalizedP = client.queryAsync(util.format("SELECT COALESCE(MAX(finalized_time), " +
                                                         "sysdate - interval '%s') as t FROM %s_tracking",
    ...
    return latestFinalizedP.then(_.flow(ut.redshiftDateFix, ut.send("toISOString")))
      .bind(this)
      .tap(function (lf) {
        debug(("Latest finalization time for " + base  + " was " + lf));
      })
      .then(function (latestFinalized) {
