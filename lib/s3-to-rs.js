...
var S3Copier = exports.S3Copier = function S3Copier(pg, s3, rs, options) {
  ...
  if(options.healthCheck) {
    debug(`Pipeline health checking enabled, etcd servers ${options.healthCheck.etcdServers}`);
  ...
  this._pollers = _.map(baseTables, baseName => {
    ...
    debug(`Creating SQS poller for ${baseName}. QueueUrl ${sqsConf.params.QueueUrl}`);
    ...
    poller.messageStream.fork()
      ...
      .filter(msg => {
        ...
        if(isSeen) {
          error(`Message with URI(s) ${msg.toURIs().join(",")} already seen before`);
        }
        return !isSeen;
      })
    ...
    poller.messageStream.observe()
      ...
      .tap((msg) => debug(`Scheduling duplicate message with URI(s) ${msg.toURIs().join(",")} for deletion`))
...
S3Copier.prototype._cleanDedup = function _cleanDedup(client, table, base, latestFinalized) {
  ...
  return client.queryAsync(`DELETE FROM ${table}_dedup WHERE processing_time < '${latestFinalized}'::timestamp - interval '${setAge}'`)
...
S3Copier.prototype._updateTracking = function _updateTracking(client, table, base, stagingTbl, latestFinalized) {
  if(this._healthChecker && !this._healthChecker.healthOk) {
    error(`Pipeline health is bad, not updating finalization time for base table ${base}`);
    return;
  }
...
S3Copier.prototype._delete = function(manif) {
  var msgs = manif.msgs;
  debug(`Scheduling ${msgs.length} messages for deletion for base table ${manif.table}. URIs are ${ut.messagesToURIs(msgs).join(",")}`);
...
S3Copier.prototype._onManifest = function _onManifest(manif) {
  ...
  var handle = () => {
    ...
    return this._tblMgr.tableFor(manif.table)
      ...
      .then(() => {
        debug(`Deleting messages for OK manifest ${manif.manifestURI} for base table ${manif.table}`);
        this._delete(manif);
      })
      .catch((err) => {
        error(`Error handling manifest ${manif.manifestURI}: ${err.stack}`);
          ...
          return Promise.join(this._tblMgr.createMetadataTbls(), this._tblMgr.createMonolithicTbls())
            .then(() => {
              if (!this._streamsInited) {
                // NOTE(ORBAT): hideous kluge to see if explicitly thunking the stream helps with odd occasional freezing
              ...
              if(this._healthChecker) {
                debug("Starting pipeline health checker");
