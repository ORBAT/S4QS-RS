...
TimeSeriesManager.prototype._listTsTables = function(name, limit) {
  ...
  return this._usingPg(function (client) {
    var queryStr = "select tablename from pg_tables where schemaname = $1" +
                   " and tablename like $2 || '_ts_%' || $3 order by tablename asc"
    ...
    .then(function(tables) {
      ...
      if(tables.length > tsOpts.maxTables) {
        var diff = tables.length - tsOpts.maxTables
          , toDrop = tables.slice(0, diff)
          , left = tables.slice(diff)
          ;
        debug("Dropping " + diff + "/" + tables.length + " time series table(s)");
        this._dropTables(toDrop).catch(function(err) {
          error("Error dropping " + tables.join(", ") + ": " + err);
        });
...
TimeSeriesManager.prototype._updateView = Promise.method(function(name, tables) {
  ...
  var colNames = _.map(opts.columns, _.flow(_.partial(_.words, _, /\w+/g), ut.get(0))).join(', ')
    , viewTables = _.takeRight(tables, opts.tablesInView || opts.maxTables)
    , viewName = name + "_view" + this._postfix
    , create = "create or replace view " + viewName + " as "
    , select = "select " + colNames + " from "
    , selects = _.map(viewTables, function(tbl) {return select + tbl}).join(' union all ')
    , query = create + selects
    ;
  ...
  return this._usingPg(function(client) {
    ...
    if(opts.latestPostfix) {
      var latestName = name + opts.latestPostfix + this._postfix ;
      debug("Creating latest time series table view '" + latestName + "' for table " + _.takeRight(tables, 1));
      ps.push(client.queryAsync("create or replace view " + latestName + " as " + select + _.takeRight(tables, 1)));
    }
    ps.push(client.queryAsync(query));
    return Promise.all(ps);
  }.bind(this)).return(viewName)
});
...
TimeSeriesManager.prototype.tsTableFor = Promise.method(function (name) {
  ...
  var now = Date.now()
    , postf = "_ts_" + ((now - now % (tsOpts.period * 1000)) / 1000).toFixed(0) + this._postfix;
  ...
  return this._usingPg(function (client) {
    ...
    .tap(function () {
      ...
      this._pruneTsTables(name).bind(this)
        .then(function (remaining) {
          return this._updateView(name, remaining).catch(function (err) {
