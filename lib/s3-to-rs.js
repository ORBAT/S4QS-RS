 ...
 *
 * @param {Object} s3 Initialized AWS S3 object
 ...
 * @type {S3Copier}
 */
var S3Copier = exports.S3Copier = function S3Copier(poller, pg, s3, copyParams, options) {
  if(!options) {
...
S3Copier.prototype._doDelete = function() {
  ...
  return this._poller.deleteMsgs(this._toDelete)
    .catch(function (err) {
      // TODO: should the _toDelete array be emptied here or not? Not clearing it will cause it to fill up if
      // lots of deletions fail for whatever reason.
      error("Error deleting messages: " + err);
    })
    .tap(function () {
      debug("Messages deleted");
...
S3Copier.prototype._onManifest = function _onManifest(manif) {
  ...
  var promise = new Promise(function (resolve) {
    debug("Got manifest " + manif.manifestURI + " for table " + manif.table);
    self._connAndCopy(manif.manifestURI, manif.table)
      .then(function (uri) {
        debug("Manifest " + uri + " COPY'd.");
        return Promise.settle([manif.delete(), self._delete(manif.msgs)])
          .then(function (settled) {
            if (settled[0].isRejected()) {
              error("Error deleting manifest " + manif.manifestURI);
            }

            if (settled[1].isRejected()) {
      ...
      .finally(function() {
        debug("Manifest " + manif.manifestURI + " handled");
        resolve(manif);
      })
    ;
  });
...
S3Copier.prototype._onMsgs = function _onMsgs(msgs) {
  ...
  this._manifestsPending = new Promise(function (resolve) {
    if(msgs.length == 0) {
      debug("No messages, scheduling new poll.");
      self._schedulePoll(nextPoll);
      resolve();
    ...
    var copyPs = Promise.map(self._dedup(msgs), function (msg) {
      return Promise.try(JSON.parse, msg.Body)
        .then(_eventToS3URIs)
        .each(function (uri) {
        return self._connAndCopy(uri)
          .then(function () {
    ...
    Promise.all(copyPs)
      .catch(function(err) {
        error("Odd error during a copy: " + err);
      })
      .done(function() {
      debug("All copied");
