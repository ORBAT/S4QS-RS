...
S3Copier.prototype._stagingToActual = function _stagingToActual(client, table, base, stagingTbl) {
  /*
   insert into [actual]
   select id, event, ts, uid, adid_md5, android_id, identity_created, platform, os_version, source_id, developer_id, country, ab_group, json_ext, sdk_version, campaigns_avail
   from
   (select [staging].*, rank() over (partition by [idCol]) as id_rank
   from [staging]
   left join [dedup] on ([idCol] = [staging].[idCol])) as staging
   where id_rank = 1 and staging.[idCol] is null
   and staging.[tsCol] >= (sysdate - interval '45 minutes');
   */
  ...
  var dedupConf = this._dedupConfsFor(base)
    , columns = _(this._options.tableConfig[base].columns) // get column definitions
    ...
    , tsc = dedupConf.timestampCol
    , dropOlder = dedupConf.dropOlderThan ?
      util.format("AND staging.%s >= (sysdate - interval '%s')", tsc, dedupConf.dropOlderThan) : ""
    , query = util.format(
      "INSERT INTO %s SELECT %s FROM "
      + "(SELECT %s.*, RANK() OVER (PARTITION BY %s.%s) AS id_rank, dedup.%s AS dedup_id "
      + "FROM %s "
      + "LEFT JOIN %s_dedup AS dedup USING (%s)) as staging "
      + "WHERE id_rank = 1 and dedup_id IS NULL "
      + "%s"
      , table, columns.join(", ")
      , stagingTbl, stagingTbl, pk, pk
      , stagingTbl
