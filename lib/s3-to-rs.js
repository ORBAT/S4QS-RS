...
TableManager.prototype._genDropColumns = function(baseName, toDrop) {
  const monolTableName = this.tableFor(baseName)
    , errorsTableName = monolTableName + "_errors"
    ;
  return toDrop.map(colName => [monolTableName, colName]).concat(toDrop.map(colName => [errorsTableName, colName]))
    .map(([tbl, col]) => `ALTER TABLE ${tbl} DROP COLUMN "${col}"`);
};
...
TableManager.prototype._genAddColumns = function(baseName, toAdd) {
  const monolTableName = this.tableFor(baseName)
    , errorsTableName = monolTableName + "_errors"
    ;
  return toAdd.map(colName => [monolTableName, colName]).concat(toAdd.map(colName => [errorsTableName, colName]))
    .map(([tbl, col]) => `ALTER TABLE ${tbl} ADD COLUMN ${this._findColumnDef(baseName, col)}`)
};
  ...
  if(toBeAltered.length) {
    ...
    try {
      ...
      yield Promise.all(_.map(alters, (statements, baseName) => {
        this._logger.info({statements: statements, baseName: baseName}, "Altering table");
