...
S3Copier.prototype._tableConfsFor = function _tableConfsFor(base) {
  return this._options.tableConfig[base];
};
...
S3Copier.prototype._updateDedupTbl = function _updateDedupTbl(client, table, base, stagingTbl) {
  /*
   ...
   INSERT INTO ads_events_dedup
   SELECT DISTINCT staging_{now}.event_id, {now} AS processing_time;
   FROM staging_{now}
   LEFT JOIN ads_events_dedup USING (event_id)
   WHERE ads_events_dedup.event_id IS NULL;
   */
};
...
S3Copier.prototype._badToErrTbl = function _badToStaging(client, table, base, stagingTbl) {
  var dedupConf = this._dedupConfsFor(base)
    ...
    , dropOlder = dedupConf.dropOlderThan ?
      util.format("OR %s.ts < (now() - interval '%s')", stagingTbl, dedupConf.dropOlderThan) : ""
    , query = util.format("INSERT INTO %s_errors "
                                   + "(SELECT %s_dedup.%s, now() as processing_time, %s.* FROM %s "
...
S3Copier.prototype._stagingToActual = function _stagingToActual(client, table, base, stagingTbl) {
  ...
  var dedupConf = this._dedupConfsFor(base)
    , pk = this._tblMgr.pkFor(base).name
    , dropOlder = dedupConf.dropOlderThan ?
      util.format("AND %s.ts < (now() - interval '%s')", stagingTbl, dedupConf.dropOlderThan) : ""
...
S3Copier.prototype._connAndCopy = function _connAndCopy(s3URI, base, table) {
  // if dedupConfig is truthy, we need to do dedup first.
  //
  // create metadata tables:

  //
  // Generate [uid], get current time to [now]
  // start transaction
  // CREATE TEMPORARY TABLE staging_[table]_[uid] (LIKE [table]);
  // do COPY to staging table
  //
  ...
  function dedupCopy(client) {
    /*
     ...
     FROM [staging]
     LEFT JOIN [table]_dedup USING (event_id)
     WHERE [table]_dedup.event_id IS NULL
     AND [staging].ts < {now} - interval '45 minutes';
     */
    ...
    return client.queryAsync(query)
      ...
      .then(this._stagingToActual.bind(this, client, table, base, stagingTbl))
      .then(this._badToErrTbl.bind(this, client, table, base))
      .return(s3URI)
      ;
  }
    ...
    return (skipAvailCheck ? Promise.resolve(true) : this._isClusterAvail())
      ...
      .then(function (avail) {
        if (avail) {
          ...
          return this._tblMgr.createMetadataTbls().then(function() {
...
TableManager.prototype._createDedupTbl = function(client, base) {
  ...
  return client.queryAsync(util.format("CREATE TABLE IF NOT EXISTS %s (%s %s DISTKEY, ts TIMESTAMP SORTKEY);",
...
TableManager.prototype.createMetadataTbls = Promise.method(function() {
  ...
  debug("Setting up deduplication metadata tables for " + tablesWithDedup.length + " tables");
  return this._usingPg(function (client) {
...
TableManager.prototype.tableFor = Promise.method(function (name) {
  ...
  // no time series table configuration, use a monolithic table. The table name is the base name + postfix
  if(!(tsOpts.period || tsOpts.maxTables || tsOpts.tablesInView)) {

    return this._usingPg(function(client) {
      return client.queryAsync(util.format("CREATE TABLE IF NOT EXISTS %s (%s) %s ", nameWithPostf,
        tsOpts.columns.join(','),
      tsOpts.tableAttrs.join(' '))).return(nameWithPostf);
    })
  }
  ...
  var stampStr;
  if (tsOpts.period < 86400) { // period is less than 24h
