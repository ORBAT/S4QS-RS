...
const S3Copier = exports.S3Copier = function S3Copier(pg, s3, rs, options) {
  ...
  this._pollers = _.map(baseTables, baseName => {
    ...
    const sqsConf = _.merge(baseSqsConf, commonSqsConf);
    const pollerOpts = sqsConf.poller || {};
    ...
    const poller = new p.Poller(sqs, pollerOpts);
...
S3Copier.prototype._copyTemplateFor = function(base) {
  if(!this._baseToTempl[base]) {
    ...
    const paramsForBase = _.omit(_.get(this._options.tableConfig[base], "copyParams") || {}, "table");
...
S3Copier.prototype._connAndCopy = function _connAndCopy(s3URI, base, table) {
  ...
  function _dedupCopy(client) {
    ...
    const latestFinalizedP = client.queryAsync(
