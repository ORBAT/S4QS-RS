...
S3Copier.prototype._connAndCopy = function _connAndCopy(s3URI,table) {
  var cancelCopy = null
    , txCtx = "COPY for " + manif.manifestURI;
  ...
  return this._usingPg(function (client) {
    ...
    .catch(Promise.CancellationError, function(e) {
      ...
      error("_connAndCopy cancelled for " + table);
      throw e;
    });
};
...
S3Copier.prototype._onManifest = function _onManifest(manif) {
  ...
  if(this._manifestsPending[manif.table] && this._manifestsPending[manif.table].isPending()) {
    error("Already have a pending manifest upload for table " + manif.table + this._tablePostfix + " (" + manif.manifestURI  + ")" + ". Waiting for it.");
...
TimeSeriesManager.prototype._createView = Promise.method(function _createView(viewName, viewTables, columnMap) {
  ...
  return this._usingPg(function (client) {
    ...
    return ut.inTransaction(client, "renaming " + viewName, function (client) {
      debug(util.format("_createView for %s in transaction", viewName));
      return client.queryAsync(util.format("create view temp_%s as %s", viewName, selects))
        .then(function() {
          client.queryAsync(util.format("alter table %s rename to old_%s", viewName, viewName))
        })
        .then(function () {
