...
S3Copier.prototype._dedupConfsFor = function _dedupConfsFor(base) {
  var conf = this._options.tableConfig[base].deduplication;
  if(!(conf.timestampCol)) {
    throw new Error("Missing timestampCol in deduplication configs for " + base);
  }
  return conf;
};
...
S3Copier.prototype._connAndCopy = function _connAndCopy(s3URI, base, table) {
  ...
  function dedupCopy(client) {
    ...
    return client.queryAsync(query)
      ...
      .then(function() {
        return Promise.join(
          this._stagingToActual(client, table, base, stagingTbl)
          , this._badToErrTbl(client, table, base, stagingTbl)
        )
      })
      .then(function() {
        return Promise.join(
...
/*      .catch(Promise.CancellationError, function(e) {
      ...
      .catch(function (err) {
        if(!(err instanceof Promise.CancellationError)) {
          ...
          return Promise.delay(1000).throw(err); // just wait a bit on the off chance the cancel requests actually work
        }
...
S3Copier.prototype.stop = function(checkAvail) {
  ...
  if(this._numPending() > 0) {
    var cancelledCopys = this._cancelAllPending();

    return Promise.settle(cancelledCopys)
...
S3Copier.prototype._cancelAllPending = function _cancelAllPending() {
  debug("Canceling pending copys for " + this._numPending() + " group(s)");
  return _.compact(_.map(_.values(this._manifestsPending), function (promise) {
    if(promise.isPending())
      return promise.cancel(); // TODO: add timeout
    return null;
  }))
};
