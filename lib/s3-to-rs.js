...
S3Copier.prototype._connAndCopy = function _connAndCopy(s3URI, txTrigger, table) {
  var self = this;
  return this._usingPg(function (client) {
    return Promise.using(ut.withTransaction(client, txTrigger), function (client) {
...
S3Copier.prototype._onManifest = function _onManifest(manif) {
  ...
  var txTrigger = {shouldCommit: true, reason: ""};



  var handle = function _handle() {

    var doCopy = _.partial(this._connAndCopy.bind(this), manif.manifestURI, txTrigger);

    return this._tsMgr.tsTableFor(manif.table).bind(this)
      .then(doCopy)
      .catch(Promise.CancellationError, function(e) {
        debug("Not deleting messages due to COPY cancel for manifest " + manif.manifestURI);
        throw e;
      })
      .then(function (uri) {
...
TimeSeriesManager.prototype.tsTableFor = Promise.method(function (name) {
  ...
  } else if (tsOpts.period < 60*60*24*31) {
