 ...
 * @type {S3Copier}
 */
var S3Copier = exports.S3Copier = function S3Copier(pg, s3, rs, options) {
  if(!options) {
  ...
  if(options.tablePostfix) {
    this._logger.info({postfix: options.tablePostfix}, "Table postfix set");
  }
  ...
  var commonSqsConf = config.has('SQS') ? config.get('SQS') : {};

  this._pollers = _.map(baseTables, baseName => {
    var baseSqsConf = options.tableConfig[baseName].SQS || {};
    var sqsConf = _.merge(baseSqsConf, commonSqsConf);
    var pollerOpts = sqsConf.poller || {};
    pollerOpts.filter = ut.nameFilterFnFor([baseName], _tableNamer);
    pollerOpts.statsd = options.statsd;
    pollerOpts.logger = this._logger;
    var sqs = new aws.SQS({region: sqsConf.region, params: sqsConf.params});
    ...
    poller.messageStream.fork()
      ...
      .filter(msg => {
        var isSeen = _.any(msg.toURIs(), this._URISeenCache.get.bind(this._URISeenCache));
        return !isSeen;
      })
      .pipe(this._uploader.inputStreamMap[baseName]);
  ...
  // each of the uploader's output streams is a Stream[Stream[Manifest]].
  this._unseenStream = $(_.map(this._uploader.outputStreams, (stream) => {
    return stream.sequence().flatMap(this._onManifest.bind(this));
  })).merge()
    .errors((err) => {
...
S3Copier.prototype._cleanDedup = function _cleanDedup(client, table, base, latestFinalized) {
  ...
  client.txLogger.info({baseName: base, dedupSetAge: setAge, latestFinalized: latestFinalized}, "Cleaning dedup table");

  var query = `DELETE FROM ${table}_dedup WHERE processing_time < '${latestFinalized}'::timestamp - interval '${setAge}'`;

  return client.queryAsync(query)
    .tap((res) => client.txLogger.info({rowCount: res.rowCount, baseName: base}, `Dedup table cleaned`))
...
S3Copier.prototype._updateTracking = function _updateTracking(client, table, base, stagingTbl, latestFinalized) {
  ...
  var dedupConf = this._dedupConfsFor(base);
  client.txLogger.info({baseName: base, dropOlderThan: dedupConf.dropOlderThan, latestFinalized: latestFinalized}, "Updating tracking table");

  var query = `INSERT INTO ${table}_tracking (processing_time, finalized_time) (
...
S3Copier.prototype._updateDedupTbl = function _updateDedupTbl(client, table, base, stagingTbl) {
  ...
  client.txLogger.info({baseName: base, stagingTbl: stagingTbl}, "Updating dedup table");

  return client.queryAsync(query).tap((res) => client.txLogger.info({rowCount: res.rowCount, baseName: base}, "Dedup table update done"))
    .get("rowCount")
...
S3Copier.prototype._badToErrTbl = function _badToStaging(client, table, base, stagingTbl, latestFinalized) {
  client.txLogger.info({baseName: base, stagingTbl: stagingTbl, latestFinalized: latestFinalized}, "Inserting duplicates and old events into error table");
  var pk = this._tblMgr.pkFor(base).name
  ...
  return client.queryAsync(query).tap((res) => client.txLogger.info({rowCount: res.rowCount, baseName: base}, "Handled duplicate/old rows"))
    .get("rowCount")
...
S3Copier.prototype._stagingToActual = function _stagingToActual(client, table, base, stagingTbl, latestFinalized) {
  client.txLogger.info({baseName: base, stagingTbl: stagingTbl, latestFinalized: latestFinalized}, "Inserting from staging to actual table");
  ...
  return client.queryAsync(query).tap((res) => client.txLogger.info({baseName: base, rowCount: res.rowCount}, "Inserted from staging to actual table"))
    .get("rowCount")
...
S3Copier.prototype._connAndCopy = function _connAndCopy(s3URI, base, table) {
  ...
  function _dedupCopy(client) {
    ...
    return latestFinalizedP.then(_.flow(ut.redshiftDateFix, ut.send("toISOString")))
      .tap((lf) => logger.trace({latestFinalized: lf}, "Found latest finalization time"))
  ...
  return this._usingPg(logger, ins.instrumentCalls("total", (client) =>
      ut.inTransaction(client, txCtx, (txClient) => {
        ...
        else return copyTo.call(this, txClient, table).return(s3URI)
          ;
      })
...
S3Copier.prototype._delete = function(manif) {
  var msgs = manif.msgs;
...
S3Copier.prototype._onManifest = function _onManifest(manif) {
  var logger = this._logger.child({manifestURI: manif.manifestURI, baseName: manif.baseName});

  logger.info("Got new manifest");

  var handle = () => {
    var doCopy = _.partial(this._connAndCopy.bind(this), manif.manifestURI, manif.baseName);
  ...
  if(this._manifestsPending[manif.baseName] && this._manifestsPending[manif.baseName].isPending()) {
    // TODO: add the waiting manifest to an obj using URI as key, then before executing 'handle' remove it in the then() handler below
    // TODO: go through the waiting manifest object when catching top-level errors in the stream, call manif.done(false)?
    logger.error("Already have a pending manifest upload for this table. Current manifest will wait");
...
S3Copier.prototype.stop = function(checkAvail) {
  if(this.started) {
    ...
    this._logger.info({checkAvail: checkAvail}, "Stopping");
  ...
  if(this._numPending() > 0) {
    return Promise.settle(_.map(_.values(this._manifestsPending), (promise) => {
      ...
      .then((settled) => {
        ...
        this._logger.info({nOk: nOk, nFail: nFail}, "Cancel done");
        if(nFail) {
          this._logger.info({reasons: _.map(byFulfilled.false, ut.send("reason"))}, "Failure reasons");
        }
      });
  }
...
S3Copier.prototype._isClusterAvail = function _isClusterAvail() {
  return this._rs.describeClustersAsync()
    .then((res) => {
      ...
      this._logger.debug({ClusterStatus: res.Clusters[0].ClusterStatus}, "Status check done");
          ...
          return Promise.join(this._tblMgr.createMetadataTbls(), this._tblMgr.createMonolithicTbls())
            .then(() => {
              if (!this._streamsInited) {
                this._unseenStream.each((mf) => this._logger.info({manifestURI: mf.manifestURI}, "Finished handling manifest"));
                this._streamsInited = true;
              }
              this._pauser.unpause();
              // this resume only needs to happen once, but it won't hurt calling it multiple times.
              // Once the stream gets going, we can use _pauser to pause/unpause everything.
              ...
              if(this._healthChecker) {
                ...
                return this._healthChecker.start();
              }
            })
            .tap(() => this._logger.info("Started"));
        } else {
 ...
 * @constructor
 */
var TableManager = exports.TableManager = function TableManager(usingPgFn, schema, postfix, logger, options) {
...
TableManager.prototype._createDedupTbl = function(client, base) {
  ...
  this._logger.info({dedupTable: table}, `Creating dedup table`);
...
TableManager.prototype._createTrackingTbl = function(client, base) {
  ...
  this._logger.info({trackingTable: table}, "Creating tracking table");
...
TableManager.prototype._createErrorTbl = function(client, base) {
  ...
  this._logger.info({errorTable: table}, `Creating error table`);
  ...
  return this._usingPg(this._logger, (client) =>
      ut.inTransaction(client, "createMonolithicTbls", (txClient) =>
        Promise.map(monoTbls, (base) => {
          ...
          this._logger.info({tableName: nameWithPostf}, "Creating table");
