...
S3Copier.prototype._cleanDedup = function _cleanDedup(client, table, base) {
  ...
  return client.queryAsync(`DELETE FROM ${table}_dedup WHERE processing_time < sysdate - interval '${setAge}'`).tap(function(res) {
...
S3Copier.prototype._updateTracking = function _updateTracking(client, table, base) {
  ...
  return client.queryAsync(`INSERT INTO ${table}_tracking (processing_time, finalized_time) VALUES (sysdate, sysdate - interval '${dedupConf.dropOlderThan}')`);
};
...
S3Copier.prototype._stagingToActual = function _stagingToActual(client, table, base, stagingTbl, latestFinalized) {
  ...
  var dedupConf = this._dedupConfsFor(base)
    ...
    , query = `INSERT INTO ${table} SELECT ${columns.join(", ")} FROM `
              ...
              + `WHERE id_rank = 1 and dedup_id IS NULL AND staging.${dedupConf.timestampCol} >= ('${latestFinalized}'::timestamp)`
    ;
  debug(query);
  return client.queryAsync(query).tap(function(res) {
...
S3Copier.prototype._connAndCopy = function _connAndCopy(s3URI, base, table) {
  ...
  function _dedupCopy(client) {
    ...
    var latestFinalizedP = client.queryAsync((`SELECT COALESCE(MAX(finalized_time), sysdate - interval '${dedupConf.dropOlderThan}') as t FROM ${table}_tracking`))
  ...
  return this._usingPg(function(client) {
    return ut.inTransaction(client, "createMonolithicTbls", function (client) {
      return Promise.map(monoTbls, function(base) {
        ...
        return client.queryAsync(`CREATE TABLE IF NOT EXISTS ${nameWithPostf} (${opts.columns.join(',')}) ${opts.tableAttrs.join(' ')}`);
      }.bind(this))
    }.bind(this));
  }.bind(this))
