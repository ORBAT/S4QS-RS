  ...
  return this._usingPg(function (client) {
    ...
    return ut.inTransaction(client, "renaming " + viewName, function (client) {
      return client.queryAsync("drop view if exists old_" + viewName)
        ...
        .then(function() {
          return client.queryAsync(util.format("alter table %s rename to old_%s", viewName, viewName)).catch(function(err) {
            error(util.format("Error renaming %s to old_%s: %s", viewName, viewName, inspect(err)));
          })
        })
        .then(function () {
