 ...
 * @param {Object} pg Connected and promisified Postgres client
...
/*  this._usingPg = function(fn) {
    return this._pgClientP.then(fn);
  }.bind(this);*/

  this._tsMgr = new TimeSeriesManager(pg, copyParams.schema, this._tablePostfix, options.timeSeries);
...
S3Copier.prototype._connAndCopy = function _connAndCopy(s3URI, table) {
  ...
  return this._pgClient.queryAsync(query).return(s3URI);
};
 ...
 * @param {*} pgClient A connected, promisified postgres client
 * @param {String} postfix A table postfix to be added to the base name _before_ the time series postfix.
 * @constructor
 */
var TimeSeriesManager = exports.TimeSeriesManager = function TimeSeriesManager(pgClient, schema, postfix, options) {
...
TimeSeriesManager.prototype._listTsTables = function(name) {
  ...
  return this._pgClient.queryAsync(queryStr, params)
    .then(function (res) {
...
TimeSeriesManager.prototype._columnsForTables = function(tableNames) {
  ...
  return this._pgClient.queryAsync(util.format("select tablename, \"column\" from pg_table_def where tablename in (%s) " +
...
TimeSeriesManager.prototype._dropTables = function(tables) {
  ...
  return this._pgClient.queryAsync(query).return(tables);
};
...
TimeSeriesManager.prototype._createView = Promise.method(function _createView(viewName, viewTables, columnMap) {
  ...
  return Promise.using(ut.getTransaction(this._pgClient), function (client) {
...
TimeSeriesManager.prototype.tsTableFor = Promise.method(function (name) {
  ...
  return this._pgClient.queryAsync(ddlStr).bind(this)
    .tap(function () {
