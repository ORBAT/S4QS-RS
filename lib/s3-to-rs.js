...
S3Copier.prototype._onManifest = function _onManifest(manif) {
  ...
  var manifPromise = this._tsMgr.tsTableFor(manif.table).bind(this)
      .then(function (table) {
  ...
  if(this._manifestsPending[manif.table] && this._manifestsPending[manif.table].isPending()) {
    ...
    pendingPromise = Promise.join(manifPromise, this._manifestsPending[manif.table]);
  } else {
    pendingPromise = manifPromise
  }
