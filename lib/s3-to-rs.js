...
TimeSeriesManager.prototype._updateView = Promise.method(function(name, tables) {
  ...
  var queriesP = columnMapP.bind(this).then(function (columnMap) {
    var qs = []
      // the set of all columns in all the tables
      , allCols = _.union.apply(null, _.values(columnMap))
      , selects = _.map(viewTables, function (table) {
        return this._selectFor(table, columnMap[table], allCols);
      }.bind(this)).join(" union all ");
    qs.push("drop view if exists " + viewName);
    qs.push("create view " + viewName + " as " + selects);
    if(opts.latestPostfix) {
      var latestViewName = name + this._postfix + opts.latestPostfix
        , latestName = _.takeRight(tables, 1)
        , latestQ = "create or replace view " + latestViewName + " as " +
                    this._selectFor(latestName, columnMap[latestName], allCols);
      qs.push("drop view if exists " + latestViewName);
      qs.push(latestQ);
    }
  ...
  return this._usingPg(function (client) {
    return Promise.reduce(queriesP, function(acc, q) {
      debug("_updateView running query " + q);
      return acc.concat(client.queryAsync(q));
    }, []);
  }).return(tables);
});
