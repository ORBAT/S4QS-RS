 ...
 * @param {Object} options.pipelineHealthCheck Pipeline health check configuration
...
var S3Copier = exports.S3Copier = function S3Copier(pg, s3, rs, options) {
  ...
  if(options.pipelineHealthCheck) {
    var etcdOptions = options.pipelineHealthCheck;
    this._etcd = Promise.promisifyAll(new Etcd(etcdOptions.etcdServers));
    this._etcdKey = etcdOptions.etcdKey;
  }
  ...
  // each of the uploader's output streams is a Stream[Stream[Promise[Manifest]]].
...
/**
 * If
 * Will return (a Promise of) true if the JSON structure at _etcdKey has the key "status" with the
 * @private
 */
S3Copier.prototype._pipelineOk = function _pipelineOk() {
