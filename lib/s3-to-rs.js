...
var S3Copier = exports.S3Copier = function S3Copier(poller, pg, s3, rs, copyParams, options) {
  ...
  this._seenStream = this._poller.messageStream.observe()
    ...
    .batchWithTimeOrCount(30000, 10).map(function (msgs) {
      debug("Deleting " + msgs + " duplicates for URIs " + ut.messagesToURIs(msgs).join(","));
...
S3Copier.prototype._cleanDedup = function _cleanDedup(client, table, base, time) {
  ...
  return client.queryAsync(util.format("DELETE FROM %s_dedup WHERE processing_time < '%s'::timestamp - interval '%s'"
    , table, time, setAge));
};

S3Copier.prototype._updateTracking = function _updateTracking(client, table, base, time) {
  var dedupConf = this._dedupConfsFor(base);
  ...
  if(dedupConf.dropOlderThan) {
    ...
    return client.queryAsync(util.format("INSERT INTO %s_tracking (processing_time, finalized_time) VALUES " +
                             "('%s'::timestamp, '%s'::timestamp - interval '%s')"
      , table
      , time, time, dedupConf.dropOlderThan
    ))
  } else return Promise.resolve();

};

S3Copier.prototype._updateDedupTbl = function _updateDedupTbl(client, table, base, stagingTbl, time) {
  ...
  var dedupConf = this._dedupConfsFor(base)
    ...
    , dropOlder = dedupConf.dropOlderThan ?
      util.format("OR %s.%s < ('%s'::timestamp - interval '%s')", stagingTbl, tsc, time, dedupConf.dropOlderThan) : ""
    , query = util.format("INSERT INTO %s_dedup "
                          + "(SELECT DISTINCT %s.%s, '%s'::timestamp as processing_time FROM %s "
                          + "LEFT JOIN %s_dedup USING (%s) "
                          + "WHERE %s_dedup.%s IS NULL %s)"
      , table
      , stagingTbl, pk, time, stagingTbl
      , table, pk
      , table, pk, dropOlder
    )
    ;
...
S3Copier.prototype._badToErrTbl = function _badToStaging(client, table, base, stagingTbl, time) {
  ...
  var dedupConf = this._dedupConfsFor(base)
    ...
    , query = util.format("INSERT INTO %s_errors "
                                   + "(SELECT %s_dedup.%s, '%s'::timestamp as processing_time, %s.* FROM %s "
                                   + "LEFT JOIN %s_dedup USING (%s) "
                                   + "WHERE %s_dedup.%s IS NOT NULL %s)"
      , table
      , table, pk, time, stagingTbl, stagingTbl
      , table, pk
      , table, pk, dropOlder
    )
    ;
...
S3Copier.prototype._stagingToActual = function _stagingToActual(client, table, base, stagingTbl, time) {
  debug("Inserting from staging to actual table for " + base);
  var dedupConf = this._dedupConfsFor(base)
    , pk = this._tblMgr.pkFor(base).name
    , dropOlder = dedupConf.dropOlderThan ?
      util.format("AND %s.%s >= ('%s'::timestamp - interval '%s')", stagingTbl, tsc, time, dedupConf.dropOlderThan) : ""
...
S3Copier.prototype._connAndCopy = function _connAndCopy(s3URI, base, table) {
  ...
  function dedupCopy(client) {
    ...
    debug("Doing deduplication for base " + base + " with time " + now);
    ...
    return client.queryAsync(query)
      ...
      .then(function() {
        return Promise.join(
          this._stagingToActual(client, table, base, stagingTbl, now)
          , this._badToErrTbl(client, table, base, stagingTbl, now)
        )
      })
      //.then(this._stagingToActual.bind(this, client, table, base, stagingTbl, now))
      //.then(this._badToErrTbl.bind(this, client, table, base, stagingTbl, now))
      .then(function() {
        return Promise.join(
          this._updateDedupTbl(client, table, base, stagingTbl, now)
          , this._updateTracking(client, table, base, now)
          , this._cleanDedup(client, table, base, now)
        )
      })
      //.then(this._updateDedupTbl.bind(this, client, table, base, stagingTbl, now))
      //.then(this._updateTracking.bind(this, client, table, base, now))
      //.then(this._cleanDedup.bind(this, client, table, base, now))
      .return(s3URI)
      ;
  }
...
S3Copier.prototype.stop = function(checkAvail) {
  ...
  if(this._numPending() > 0) {
    ...
    var cancelledCopys = _.map(_.values(this._manifestsPending), function (promise) {
...
var TableManager = exports.TableManager = function TableManager(usingPgFn, schema, postfix, options) {
  ...
  this._options = options;
  this._baseNames = _.keys(options);
  this._schema = schema;
