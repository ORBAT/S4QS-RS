...
var S3Copier = exports.S3Copier = function S3Copier(poller, pg, s3, rs, copyParams, options) {
  ...
  if(!this._copyParams.args) {
    this._copyParams.args = [];
  }

  this._copyParams.args.push("MANIFEST");
...
S3Copier.prototype._connAndCopy = function _connAndCopy(s3URI, table) {
  ...
  return this._getPgClient(function (client) {
...
S3Copier.prototype._createTSTable = function(name) {

  var now = Date.now()
    , postf = "_" + ((now - now % (this._timeSeries.period * 1000)) / 1000).toFixed(0)
    , tableName = this. schema + "." + name + postf
    , ddlStr = util.format("CREATE TABLE IF NOT EXISTS %s (%s) %s "
      , tableName, this._timeSeries.tableDef.columns.join(',')
      , this._timeSeries.tableDef.tableAttrs.join(',')
    )
};
