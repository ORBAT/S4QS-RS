...
var S3Copier = exports.S3Copier = function S3Copier(poller, pg, s3, rs, options) {
  ...
  // stream messages to the manifest uploader's input stream
  ...
  })).merge() // takes the Stream[Stream[Promise[Manifest]]], flattens one level into Stream[Promise[Manifest]]
    .mergeWithLimit(maxParallel) // only handles maxParallel at a time
    .errors((err) => {
    this._errIncr();
