...
TimeSeriesManager.prototype._pruneTsTables = Promise.method(function(name, tables) {
  ...
  if(tables.length > tsOpts.maxTables) {
    ...
    debug("Dropping " + diff + "/" + tables.length + " time series table(s). New amount " + left.length);
...
TimeSeriesManager.prototype._updateView = Promise.method(function(name, tables) {
  ...
  // get time series table column names by splitting the columns option by words (\w+) and then getting the first part.
  // This is done to avoid having to do a SELECT * which apparently performs worse than explicitly listing column names
  ...
  debug("Creating view " + viewName + " with " + viewTables.length + " tables (max " + (opts.tablesInView || opts.maxTables) + ")");
  return this._usingPg(function(client) {
    var ps = [];
