...
var S3Copier = exports.S3Copier = function S3Copier(pg, s3, rs, options) {
  ...
  this._unseenStream = $(_.map(this._uploader.outputStreams, (stream, base) => {
    // returns a Stream[Promise[Manifest]]
    return stream.sequence().map(this._onManifest.bind(this))
  })).merge().merge() // takes the Stream[Stream[Promise[Manifest]]], flattens one level into Stream[Promise[Manifest]]
    .errors((err) => {
    this._errIncr();
